...
class DataPipeline:
    ...
    def fetch_pairs(self):
        ...
        **ASSUMES** load_and_aggregate_m1 is modified to SAVE raw M1 CSVs
        ...
        for symbol in self.symbols_m1:
            ...
            try:
                # This call *must* also save the raw M1 CSV to tick_data/m1/symbol_M1.csv
                # The filename convention needs to match what resample_m1_to_htf expects.
                # Example filename: OANDA_EUR_USD_M1_YYYYMMDD_YYYYMMDDHHMM.csv
                # Modify finnhub_data_fetcher.py accordingly.
                fetch_result = load_and_aggregate_m1(symbol, start_dt, end_dt)
                if fetch_result['status'] == 'ok':
                    _log(f"Fetch successful for {symbol}.")
                    successful_fetches += 1
                    # --- Modification needed in finnhub_data_fetcher ---
                    # Add logic here (or within load_and_aggregate_m1) to save
                    # the raw M1 dataframe (before aggregation) to a CSV file
                    # in self.m1_data_dir. Example:
                    # raw_m1_df = fetch_result.get('raw_m1_data') # Assume fetcher returns this
                    # if raw_m1_df is not None and not raw_m1_df.empty:
                    #     safe_symbol_name = symbol.replace(":", "_").replace("/", "_")
                    #     filename = f"{safe_symbol_name}_M1_{start_dt:%Y%m%d}_{end_dt:%Y%m%d%H%M}.csv"
                    #     filepath = self.m1_data_dir / filename
                    #     raw_m1_df.to_csv(filepath)
                    #     _log(f"Saved raw M1 data to {filepath}")
                    # else:
                    #     _log(f"WARN: Raw M1 data not saved for {symbol} (not returned by fetcher?).")
                    # --- End Modification Placeholder ---
                else:
    ...
    def resample_htf(self):
        ...
        if not RESAMPLER_AVAILABLE:
            ...
            return
        # This assumes M1 CSV files were saved by the fetch_pairs step
        # to the 'tick_data/m1/' directory.
        _log(f"Resampling all M1 datasets found in {self.m1_data_dir} -> HTF...")
        try:
            # Ensure the resampler knows where to read M1 files and write output
            # Modify resample_m1_to_htf.py if needed to accept paths or use these defaults
            # Example (conceptual modification needed in resample_m1_to_htf.py):
            # resample_all_symbols_parallel(m1_dir=str(self.m1_data_dir), output_dir=str(self.ohlcv_data_dir))

            # Assuming resample_m1_to_htf uses default paths matching here:
            # M1_DATA_DIR = "tick_data/m1/"
