...
class Settings(BaseSettings):
    ...
    def ensure_directory_exists(cls, v, field):
        ...
        try:
            ...
            raise ValueError(f"Unable to create directory '{path}' for {field.name}: {e}")
        return path

    @validator("MKDOCS_CONFIG_PATH", pre=True)
    def validate_mkdocs_config_exists(cls, v):
        import os
        if not os.path.isfile(v or ""):
            raise ValueError(f"MkDocs config file not found at path: {v}")
        return v

    # -----------------------------------------------------------------------------
    # Validators above ensure filesystem integrity for env-driven paths and config.
    # Add new validators here as you introduce additional settings.
    # -----------------------------------------------------------------------------
