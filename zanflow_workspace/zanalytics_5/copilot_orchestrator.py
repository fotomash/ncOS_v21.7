...
def handle_price_check(pair: str, timestamp_str: str, tf: str = 'm15', strategy_variant: Optional[str] = None) -> dict:
    ...
    try:
        ...
        for tf_key, df_orig in all_tf_data.items():
            if df_orig is None or df_orig.empty: enriched_tf_data[tf_key] = df_orig; continue
            df_temp = df_orig.copy(); tf_str_for_tagging = tf_key.upper()
            try:
                # Apply Standard Indicators
                if calculate_standard_indicators: df_temp = calculate_standard_indicators(df_temp, tf=tf_str_for_tagging, config=indicator_config)
                # Apply SMC Zones
                if tag_smc_zones: df_temp = tag_smc_zones(df_temp, tf=tf_str_for_tagging)
                # Apply Liquidity Sweeps (NEW)
                if tag_liquidity_sweeps: df_temp = tag_liquidity_sweeps(df_temp, tf=tf_str_for_tagging, config=liquidity_config)
        ...
        if use_advanced_flow:
            ...
            if run_advanced_smc_strategy:
                analysis_results = run_advanced_smc_strategy( enriched_tf_data=enriched_tf_data, strategy_variant=effective_variant, target_timestamp=target_dt_utc, symbol=pair, wyckoff_context=wyckoff_result )
            else:
                log_info("Advanced SMC Orchestrator module not loaded. Falling back.", "ERROR"); analysis_results = run_full_analysis(enriched_tf_data, pair, timestamp_str, tf, "Default_Fallback")
                if isinstance(analysis_results.get("analysis"), dict): analysis_results["analysis"]["wyckoff_result"] = wyckoff_result
        else:
            log_info(f"Routing to Legacy/Default Analysis Flow (Variant: {strategy_variant or 'None Specified'}).")
            analysis_results = run_full_analysis(enriched_tf_data, pair, timestamp_str, tf, strategy_variant or "Default")
    ...
    for pair in pairs:
        ...
        if result.get("status") == "ok":
            ...
            if alert_reason:
                 ...
                 alert_payload = {
                     "alert_reason": alert_reason, "pair": result.get("pair"), "timeframe": result.get("timeframe"),
                     "strategy": result.get("strategy_variant_used"), "timestamp": result.get("target_time"),
                     "htf_bias": result.get("analysis", {}).get("smc_structure", {}).get("htf_bias"),
                     "phase": result.get("analysis", {}).get("phase_result", {}).get("phase"),
                     "wyckoff_phase": result.get("analysis", {}).get("wyckoff_result", {}).get("current_phase"), # Add Wyckoff Phase
